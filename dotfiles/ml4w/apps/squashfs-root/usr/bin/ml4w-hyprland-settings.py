...
class MyApp(Adw.Application):
    ...
    def getKeywordValue(self, keyword):
        ...
        value = out_arr[0] if out_arr else ""
    
        int_val = None  # Ensure int_val is always defined
        float_val = None
        custom_val = None
        ...
        if "custom type" in value:
            ...
            int_val = custom_val.split(" ")[0]  # This reassigns int_val
    
        if self.rowtype.get(keyword) == "SpinRowFloat":
            value = int(float(float_val) * 10) if float_val else 0
        elif self.rowtype.get(keyword) == "ColorRow":
            colorhex = custom_val.split(" ")[0] if custom_val else "#000000"
            value = colorhex
        elif self.rowtype.get(keyword) == "SwitchRow":
            if int_val is not None and int_val == "1":
                value = True
            else:
                value = False
        else:
	        value = int(int_val) if int_val is not None else 0
